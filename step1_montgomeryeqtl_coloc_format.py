...
def pop_shared_variants_colocformat(aa_filepath, ea_filepath, chromrange, pval, tissue, chrom_X=False):
    ...
    print("\ntotal cis_genes counts :  {}".format(sum(total_cis_genes)))
...
if __name__ == "__main__":
    ...
    hg19_eqtl_coloc_dfnew = hg19_eqtl_coloc_df.drop(["newsnp"], axis=1)
    hg19_eqtl_coloc_dfnew.to_csv(join(output_path, tissue + "_coloc_betas_ses_ONLYshared_eQTLnogwas_1e4.txt"), sep="\t", index=False, header=True )
...
correlation.to_csv(join(output_path, tissue + "_ea_aa_cis_effects_correlation.txt"), sep="\t", header=True)


#Stats:
In [50]: hg19_eqtl_coloc_df.shape
Out[50]: (82436733, 9)

In [51]: eQTL_gwas_coloci.shape
Out[51]: (58282781, 10)

In [52]: eQTL_gwas_coloci.head()
Out[52]:
          hg19_snp  slope_EA  slope_AA  slope_se_EA  ...  maf_AA     phenotype_id      beta        se
0  chr1:731718:T:C  0.084009 -0.017718     0.055651  ...   0.051  ENSG00000187583  0.001958  0.003539
1  chr1:731718:T:C  0.023397  0.008999     0.062022  ...   0.051  ENSG00000187608  0.001958  0.003539

In [53]: len(eQTL_gwas_coloci["phenotype_id"].unique())
Out[53]: 11888

In [54]: len(hg19_eqtl_coloc_df["phenotype_id"].unique())
Out[54]: 11891

In [55]: len(eQTL_gwas_coloci["hg19_snp"].unique())
Out[55]: 5755733

In [56]: len(hg19_eqtl_coloc_df["hg19_snp"].unique())
Out[56]: 8183035
