...
export PREFIX5="EBV_LCL_Binary_cisgenebased_500kb_colocthresh075_WITHbaseONLYwithPPH0to4"
export PREFIX6="EBV_LCL_Binary_cisgenebased_500kb_colocthresh075_WITHbaseONLYwithPPH3plus4"
export PREFIX7="EBV_LCL_Binary_cisgenebased_500kb_colocthresh075_WITHbaseONLYwithPPH4"
...
sbatch -p $PARTITION -c $CORE -t $TIME --mem=$MEMORY -J "${JOBNAME}_5" -o "${OUTFILE}.5" -e "${ERRORFILE}.5" --mail-type "$MAILTYPE" --mail-user "$MAILUSER" $SCRIPTPATH/step1_WITHbaseONLYwithPPH0to4.py ${PREFIX5} ${ANNOTPATH} ${WORKDIR};

#heritability enrichment with base only and PPH3 plus PPH4
sbatch -p $PARTITION -c $CORE -t $TIME --mem=$MEMORY -J "${JOBNAME}_6" -o "${OUTFILE}.6" -e "${ERRORFILE}.6" --mail-type "$MAILTYPE" --mail-user "$MAILUSER" $SCRIPTPATH/step1_WITHbaseONLYwithPPH3plus4.py ${PREFIX6} ${ANNOTPATH} ${WORKDIR};

#heritability enrichment with base only and PPH4
sbatch -p $PARTITION -c $CORE -t $TIME --mem=$MEMORY -J "${JOBNAME}_7" -o "${OUTFILE}.7" -e "${ERRORFILE}.7" --mail-type "$MAILTYPE" --mail-user "$MAILUSER" $SCRIPTPATH/step1_WITHbaseONLYwithPPH4.py ${PREFIX7} ${ANNOTPATH} ${WORKDIR};
