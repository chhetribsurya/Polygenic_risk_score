...
for chrom in {1..22};do echo "processing chr:${chrom}..."; grep "chr${chrom}_" ../sorted.all.AFR.Me
ta.eQTL.genPC.nominal.maf05.mvmeta.fe.txt > AA/chr${chrom}_montgoeQTL_AA.txt; done

for chrom in {1..22};do echo "processing chr:${chrom}..."; grep "^\b${chrom}\b" ../sorted.dist.hwe.af.EUR_META.eQTL.nominal.hg38a.copy.txt > EUR/chr${chrom}_montgoeQTL_EUR.txt; done
...
aa_filepath = "/Users/suryachhetri/datasets/prs_project/gtex/montgomery_eQTLs"
...
gwas_stats = "/Users/suryachhetri/datasets/prs_project/final_hg19/EBV_LCL/4079_irnt.dbp.gwas.imputed_v3.both_sexes.tsv"
...
output_path = "/Users/suryachhetri/datasets/prs_project/final_hg19/EBV_LCL_DBP"
...
def pop_shared_variants_betabased(aa_filepath, ea_filepath, chromrange, pval, tissue, chrom_X=False):
    ...
    for chrom in chrom_list:
        ...
        # Drop nans if any
        shared_variants.dropna(inplace=True)
        ...
        shared_variants_df = shared_variants.loc[:,["phenotype_id", "variant_id", "pval_nominal_EA", "pval_nominal_AA", "slope_EA", 
                                                    "slope_AA", "ZScore_EA", "ZScore_AA", "maf_EA", "maf_AA",
        ...
        shared_vars_select["newsnp"] = shared_vars_select["variant_id"].str.replace("_", ":")
...
if __name__ == "__main__":
    ...
    default_format.to_csv(join(output_path, tissue + "_prs_format_gwasbaselineDBP.txt"), sep="\t", index=False, header=True)


    """From Coloc analysis - filtering for similar cis-effects"""
    coloc_output = "/Users/suryachhetri/datasets/prs_project/coloc_output/Cells_EBV-transformed_lymphocytes_genecoloc_betas_ses_eqtlgwas_1e4.txt"

    # processing variants based on thresh
    coloc_df = pd.read_csv(coloc_output, sep=" ")

    #for val in np.arange(0, 1, 0.1):
    # coloc posterior prob for causal variant thres
    coloc_pp_df = coloc_df[coloc_df["PP.H4.abf"] > 0.5]
    coloc_pp_df = coloc_df[coloc_df["PP.H4.abf"] > 0.1]

    # eQTls with phenotype id for coloc based filter
    eqtl_unfilt_df = cis_eqtl_unfilt_df.copy()

    # intersect coloc phenotype id with shared-eQTL phenotype id
    eqtl_coloc_df = pd.merge(eqtl_unfilt_df, coloc_pp_df.loc[:, "phenotype_id"], on=["phenotype_id"])
    
    # Dframe based genomelift of hg38 GTEx tissues to hg19:
    #hg19_eqtl_filt_df = liftover(eqtl_filt_df)
    hg19_eqtl_all_df = liftover(eqtl_coloc_df)
    ...
    #filename_thresh = 0.1 #for naming purpose only
    #filt_eqtl_prs_format = prs_format(filt_eQTL_gwas_loci, filename_thresh)
    unfilt_eqtl_prs_format = prs_format(all_eQTL_gwas_loci, "colocfilt0.1")
    #print(val)

    # coloc_df = pd.read_csv(coloc_output, sep=" ")
    # coloc_df[coloc_df["PP.H4.abf"] > 0.5]
